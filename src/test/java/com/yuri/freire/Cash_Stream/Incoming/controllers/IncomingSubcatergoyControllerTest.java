...
class IncomingSubcatergoyControllerTest {
    ...
    void setUp(){
        ...
        categoryServiceMock.createIncomingCategory(IncomingCategoryRequestCreator.createIncomingCategoryRequest(), UserCreator.createValidUser().getUsername());
        ...
        BDDMockito.when(subcategoryServiceMock.findAllSubcategory(ArgumentMatchers.any(), ArgumentMatchers.any()))
                .thenReturn(subcategoryPage);

        BDDMockito.when(subcategoryServiceMock.findAllByCategoryName(ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.any()))
                        .thenReturn(subcategoryPage);

            BDDMockito.when(requestMock.getRequestURI())
                    .thenReturn("/subcategory-test-path");

        BDDMockito.when(subcategoryServiceMock.createIncomingSubcategory(ArgumentMatchers.any(), ArgumentMatchers.any()))
                .thenReturn(subcategory);

        BDDMockito.when(subcategoryServiceMock.deleteBySubcategoryId(ArgumentMatchers.anyInt(), ArgumentMatchers.any()))
    ...
    void findAll_ReturnsListOfIncomingSubcategoryInsidePageObject_WhenSuccessfull(){
        Page<IncomingSubcategoryResponse> subcategoryPage = subcatergoyController.findAllIncomingSubcategory(null, requestMock, userDetailsMock).getBody().getData();
    ...
    void findAllByCategoryName_ReturnsAListOfSubcategoryByCategoryNameInsideOfPageObject_WhenSucessfull(){
        ...
        Page<IncomingSubcategoryResponse> subcategoryPage = subcatergoyController.findAllByCategoryName(expectedCategoryName, null, requestMock, userDetailsMock).getBody().getData();
        Assertions.assertThat(subcategoryPage.toList())
                .isNotEmpty()
                .hasSize(1);
    ...
    void createIncomingSubcategory_ReturnsIncomingSubcategory_WhenSuccessfull(){
        IncomingSubcategoryResponse savedSubcategory = subcatergoyController.createIncomingSubcategory(IncomingSubcategoryRequestCreator.createIncomingSubcategoryRequest(), requestMock, userDetailsMock)
                .getBody().getData();
    ...
    void deleteByCategoryId_RemovesCategory_WhenSuccessfull(){
        ResponseEntity<ApiResponse<String>> deletedSubcategory = subcatergoyController.deleteByIncomingSubcategoryId(1, requestMock, userDetailsMock);
