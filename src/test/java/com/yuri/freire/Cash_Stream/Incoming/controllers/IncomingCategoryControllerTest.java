...
class IncomingCategoryControllerTest {
    ...
    void setUp(){
        ...
        BDDMockito.when(categoryServiceMock.createIncomingCategory(ArgumentMatchers.any(IncomingCategoryRequest.class), ArgumentMatchers.any()))
                .thenReturn(IncomingCategoryCreator.createValidCategory());

        BDDMockito.when(categoryServiceMock.deleteByCategoryId(ArgumentMatchers.anyInt(), ArgumentMatchers.any()))
    ...
    void findAll_ReturnsListOfIncomingCategoryInsidePageObject_WhenSuccessfull(){
        ...
        Page<IncomingCategoryResponse> categoryPage = categoryController.findAllIncomingCategory(null, requestMock, userDetailsMock).getBody().getData();
    ...
    void createIncomingCategory_ReturnsIncomingCategoryResponse_WhenSuccessfull(){
        IncomingCategoryResponse category = categoryController.createIncomingCategory(IncomingCategoryRequestCreator.createIncomingCategoryRequest(),requestMock, userDetailsMock)
                .getBody().getData();
    ...
    void deleteByCategoryId_RemovesCategory_WhenSuccessfull(){
        ResponseEntity<ApiResponse<String>> deletedCategory = categoryController.deleteByCategoryId(1, requestMock, userDetailsMock);
